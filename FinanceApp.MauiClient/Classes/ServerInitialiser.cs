...
public class ServerInitialiser(ServerConnection serverConnection) : ConnectionInitialiser
{
    protected override async Task<bool> IsCompatibleAsync()
    {
        try {
            ...
            CompareVersionResponse response = await serverConnection.SendMessageAsync<CompareVersion, CompareVersionResponse>(request);

            if (request.SemanticVersion.IsCompatible(AppInfo.Version)) {
                return true;
            } else {
                await Shell.Current.DisplayAlert("Version Issue", $"Server version {response.SemanticVersion} is not compatible with {request.SemanticVersion}", "OK");
                return false;
            }
        } catch {
