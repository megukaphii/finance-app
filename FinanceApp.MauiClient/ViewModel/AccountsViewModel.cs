...
public partial class AccountsViewModel(ServerConnection serverConnection) : BaseViewModel(serverConnection)
{
    ...
    public async Task LoadAccounts()
    {
        try {
            ...
            GetAccountsResponse response = await ServerConnection.SendMessageAsync<GetAccounts, GetAccountsResponse>(request);
    ...
    private async Task SelectAccount(Account account)
    {
        try {
            ...
            SelectAccountResponse response = await ServerConnection.SendMessageAsync<SelectAccount, SelectAccountResponse>(request);
            if (response.Success) {
                await Shell.Current.DisplayAlert("Created Transaction", $"Successfully set account",
                    "OK");
            }
        } catch (ResponseException<SelectAccount> ex) {
            if (!string.IsNullOrEmpty(ex.Response.Id.Error)) {
                PageError = ex.Response.Id.Error;
            }
        } catch (Exception ex) {
            await ServerConnection.DisconnectAsync();
            await Shell.Current.GoToAsync($"//{nameof(Login)}", true);
    ...
    {
        return Shell.Current.GoToAsync(nameof(AccountCreate), true);
    }
