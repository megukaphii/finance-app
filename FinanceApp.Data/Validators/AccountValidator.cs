	...
	public Task<bool> ValidateAsync(IRequest request)
	{
		...
		if (request is ISingleAccount validateAgainst) {
			if (validateAgainst.Name.Value.Length < MinNameLength) {
				validateAgainst.Name.Error =
					$"{nameof(validateAgainst.Name)} should be more than {MinNameLength} characters";
				failure = true;
			} else if (validateAgainst.Name.Value.Length > MaxNameLength) {
				validateAgainst.Name.Error =
					$"{nameof(validateAgainst.Name)} should be less than {MaxNameLength} characters";
				failure = true;
			}

			if (validateAgainst.Description.Value.Length < MinDescriptionLength) {
				validateAgainst.Description.Error =
					$"{nameof(validateAgainst.Description)} should be more than {MinDescriptionLength} characters";
				failure = true;
			} else if (validateAgainst.Description.Value.Length > MaxDescriptionLength) {
				validateAgainst.Description.Error =
					$"{nameof(validateAgainst.Description)} should be less than {MaxDescriptionLength} characters";
				failure = true;
			}
		}
